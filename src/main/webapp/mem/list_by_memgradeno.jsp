...
function send_mail() {
	...
	function search_mem() {
		...
		location.href = './list_by_memgradeno.do?memgradeno=' + ${param.memgradeno} 
		             +'&col=' + col + '&word=' + word + '&recordPerPage=' + recordPerPage + '&nowPage=1'
	}
	
	// 검색 취소
	function search_cancle() {
		location.href = './list_by_memgradeno.do?memgradeno=' + ${param.memgradeno} + '&nowPage=1'
	}
...
<jsp:include page="/menu/top.jsp" />
  ...
  <ASIDE style='float: left;'>
    회원등급 > ${memgradeVO.gname }
  </ASIDE>
  
  <ASIDE style='float: right;'>
    <A href="javascript:location.reload();">새로고침</A>
    <span class='menu_divide' >│</span> 
    <A href='./create_agr.do'>회원등록</A>
  </ASIDE> 
  
<DIV class='title_line'>${memgradeVO.gname }등급 회원 목록</DIV>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-default" data-dismiss="modal" onclick='window.location.reload();'>확인</button>
        </div>
      </div>
    </div>
  </div>
...
<c:set var="word" value='${pagingVO.word }' />
  ...
  <TR>
    ...
    <TD class='td'><A href="./readById.do?id=${memVO.id}">${memVO.id}</A></TD>
    <TD class='td'>
    ...
    <TD class='td' style='text-align: center;'>
      <A href="./update_passwd.do?memno=${memno}"><IMG src='./images/passwd.png' data-toggle="tooltip" title='패스워드 변경'></A>
